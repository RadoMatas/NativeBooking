...
export function BookingProvider({ children }: { children: React.ReactNode }) {
  ...
  const updateBookingStatus = async (id: string, newStatus: string, reason?: string) => {
    const updatePayload: Record<string, any> = {
      ...
      customerNotification:
        newStatus === 'Confirmed'
          ? 'Your work order was confirmed and scheduled.'
          : newStatus === 'Cancelled'
          ? `Your work order was cancelled by dispatch${reason ? `: ${reason}` : '.'}`
          : null,
  ...
  const acceptReschedule = async (id: string) => {
    ...
    const updates: Record<string, any> = {
      ...
      status: 'Confirmed',
      adminStatus: null,
      customerNotification: `Your reschedule request was approved. Your job is scheduled for ${
  ...
  const declineReschedule = async (id: string) => {
    ...
    const updates: Record<string, any> = {
      ...
      status: 'Confirmed',
      adminStatus: null,
      customerNotification: `Your reschedule request was declined. Your original job on ${
  ...
  const cancelBooking = async (id: string, reason?: string) => {
    const updates: Record<string, any> = {
      ...
      customerNotification: 'Your work order was cancelled.',
  ...
  const updateSessionDetails = async (
    ...
    const booking = bookings.find((b) => b.id === id)
    const notifMsg = `Dispatch updated site notes & work order details for your ${booking?.service || 'job site'} on ${booking?.date || 'date'}.`
