...
export function BookingProvider({ children }: { children: React.ReactNode }) {
  ...
  const INITIAL_BOOKINGS: Booking[] = [
    {
      ...
      customerPhone: "+1 (214) 555-0988",
      service: "English General Course",
      date: getPastDate(3),
      time: "10:00",
      status: "Confirmed",
      notes: "Enrolling for TOEFL preparation prep assessment.",
      priceCharged: 50,
      adminNotesForCustomer: "Completed grammar test with 92%. Assigned workbook Chapter 1-3.",
      internalAdminNotes: "Upper-Intermediate level student. Strong speech, minor spelling adjustments needed.",
      artistId: "sarah",
      artistName: "Prof. Sarah Jenkins",
      depositAmount: 0,
      depositPaid: false
    },
    {
      ...
      customerEmail: "customer@test.com",
      customerPhone: "+1 (214) 555-0988",
      service: "Spanish Intensive Class",
      date: getFutureDate(2),
      time: "14:00",
      status: "Confirmed",
      notes: "Needs focus on professional Spanish email conversation vocabulary.",
      artistId: "alan",
      artistName: "Prof. Alan Sterling",
      depositAmount: 0,
      depositPaid: false
    },
    {
      ...
      customerEmail: "customer@test.com",
      customerPhone: "+1 (214) 555-0988",
      service: "Private SAT Prep Tutoring",
      date: getFutureDate(5),
      time: "11:00",
      status: "Pending",
      notes: "SAT math test practice book review session.",
      adminStatus: "New",
      artistId: "alan",
      artistName: "Prof. Alan Sterling",
      depositAmount: 0,
  ...
  const updateBookingStatus = async (id: string, newStatus: string, reason?: string) => {
    const updatePayload: Record<string, any> = {
      ...
      customerNotification:
        newStatus === 'Confirmed'
          ? 'Your course enrollment seat was confirmed.'
          : newStatus === 'Cancelled'
          ? `Your booking was cancelled by academic admin${reason ? `: ${reason}` : '.'}`
          : null,
  ...
  const acceptReschedule = async (id: string) => {
    ...
    const updates: Record<string, any> = {
      ...
      status: 'Confirmed',
      adminStatus: null,
      customerNotification: `Your reschedule request was accepted. Your session is confirmed for ${
  ...
  const declineReschedule = async (id: string) => {
    ...
    const updates: Record<string, any> = {
      ...
      status: 'Confirmed',
      adminStatus: null,
      customerNotification: `Your reschedule request was declined. Your original session on ${
        booking.originalDate || booking.date
      } at ${booking.originalTime || booking.time} remains confirmed.`,
  ...
  const cancelBooking = async (id: string, reason?: string) => {
    const updates: Record<string, any> = {
      ...
      customerNotification: 'Your course seat was cancelled.',
  ...
  const updateSessionDetails = async (
    ...
    const booking = bookings.find((b) => b.id === id)
    const notifMsg = `Your instructor has updated the syllabus & attendance notes for your ${booking?.service || 'course session'} on ${booking?.date || 'past date'}.`
